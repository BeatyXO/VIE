...
class VerifiableImpactEscrow(gl.Contract):
    owner: Address
    next_intent_id: u256
    open_intents: u256
    ...
    def send_callback(self, intent_id: u256) -> None:
        ...
        self._write_intent(intent_id, rec)
        IVIEConsumer(callback).emit(on="finalized").on_service_resolved(
            intent_id,
    ...
    def _resolution_prompt(self, spec: str, evidence_policy: str, deliverables: str, evidence_bundle: str) -> str:
        return (
            "You are a GenLayer validator judging evidence for a verifiable impact escrow. "
            ...
            "Use SATISFIED only when evidence clearly demonstrates the claimed impact. "
